...
class YourPlacesNotifier extends StateNotifier<List<Place>> {
  ...
  Future<void> loadPlaces() async {
    ...
    final data =
        await db.query('user_places'); //return list of maps, so each row
    final loadlist = data.map(
      (row) {
        return Place(
          id: row['id'] as String,
  ...
  void removePlace(String id) async {
    state = state.where((listplace) {
      return listplace.id != id;
    }).toList();
...
final placesProvider = StateNotifierProvider<YourPlacesNotifier, List<Place>>(
